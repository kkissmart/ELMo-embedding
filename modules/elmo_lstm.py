...
class ElmoLstm(_EncoderBase):
    """
    A stacked, bidirectional LSTM which stacks cell
    defined in lstm_cell_with_projection
    with highway layers between the inputs to layers.
    The inputs to the forward and backward directions
    are independent - forward and backward
    states are not concatenated between layers.
    Additionally, this LSTM maintains its `own` state,
    which is updated every time ``forward`` is called.
    It is dynamically resized for different batch sizes
    and is designed for use with non-continuous inputs
    (i.e inputs which aren't formatted as a stream,
    such as text used for a language modelling task,
    which is how stateful RNNs are typically used).
    This is non-standard, but can be thought of
    as having an "end of sentence" state,
    which is carried across different sentences.
    Parameters
    ----------
    ...
    def _lstm_forward(self,
        ...
        if initial_state is None:
            # condition the first lstm in the sequence
            # len(self.forward_layers) =  # of total layers
        ...
        for layer_index, state in enumerate(hidden_states):
            ...
            if state is not None:
                ...
                forward_state = None
                backward_state = None
            # forward_output_sequence is the
